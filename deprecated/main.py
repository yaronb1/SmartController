...
def main():
    ...
    pTime,cTime=0,0
    hand = None
    # imgFist = cv2.imread(controller.resource_path('fist2.png'),-1)
    # optionsHome = cv2.imread(controller.resource_path('options2.png'),-1)
    # optionsYou = cv2.imread(controller.resource_path('optionsYou.png'),-1)
    # optionsLights = cv2.imread(controller.resource_path('optionsL.png'),-1)
    # drawI = cv2.imread(controller.resource_path('draw_main.png'),-1)
    ...
    #horn = Gesture(detector, 'devil horn',f)
    ...
    with open('/home/yaron/PycharmProjects/SmartController/variables.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    ...
    while True:
        ...
        try:finalI = screen.ui_handler(img, brush.imgCanvas,hand)
            ...
            #finalI = cv2.rotate(finalI,cv2.ROTATE_90_CLOCKWISE)
            #finalI = controller.overlay_transparent(img, ui, 0, 0, (1280,720))
